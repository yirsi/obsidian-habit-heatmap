...
export class DashboardRenderer {
    ...
    renderDashboard(store: HabitStore, stats: StatConfig[]): string {
        const globalHeaderHtml = this.renderGlobalXP(store.global);
	 ...
	 *  REMOVED AND MERGED INTO MAIN XP BAR
	 */
	private renderQuestBar(globalData: GlobalData): string {
		const completionPercentage =
			globalData.quest.total > 0
    ...
    private renderGlobalXP(globalData: GlobalData): string {
        const { levelData, todayXp, title, isPerfectDay, quest } = globalData;
        ...
        const levelTitleLabel = `Lvl ${levelData.level}: ${title}`;
        const questLabel = `Daily Quests: ${quest.completed}/${quest.total}`;
        ...
        return `
            <div class="hhm-xp-wrapper ${glowClass}">
                <div class="hhm-xp-label">
                    <span>${levelTitleLabel}</span>
                    <span>${questLabel}</span>
                </div>
                <div class="hhm-xp-bar-outer" style="height:14px; position: relative; margin: 8px 0;">
                ...
                <div class="hhm-xp-stats-label" style="font-size:0.8em; opacity:0.8; display: flex; justify-content: space-between;">
                    <span>${xpText}</span>
                    ${xpBonusHtml}
                </div>
            </div>
	...
	private renderCard(stat: StatConfig, habit: HabitData | undefined): string {
		...
		// Success is defined by the goal direction
		const isDone =
			stat.goal === "up"
				? habit.currentToday > 0
				: habit.currentToday === 0;

		// UI state flags
		const statusClass = stat.streakEnabled && isDone ? "hhm-card-done" : "";
	...
	private renderLogOverlay(stat: StatConfig, currentVal: number): string {
		...
		// Constraint-based UI: if range is small, use buttons. Else, stepper.
		const range = stat.boundaries.max - stat.boundaries.min;

		if (range <= 7) {
			let buttons = "";
			for (let i = stat.boundaries.min; i <= stat.boundaries.max; i++) {
				const activeClass = i === currentVal ? "is-active" : "";
				buttons += `<button class="hhm-log-btn rating-btn ${activeClass}" data-val="${i}">${i}</button>`;
			}
			uiHtml = `<div class="hhm-log-strip">${buttons}</div>`;
		} else {
			// Assume it's a "Macro" time tracker if max is very high
			const isMacro = stat.boundaries.max > 24;
			const min = stat.boundaries.min;
			const max = stat.boundaries.max;
			const isDisabled = (step: number) =>
				currentVal + step < min || currentVal + step > max
					? "disabled"
					: "";
		...
		const saveClass =
			range <= 7
				? "hhm-log-save rating-save"
				: "hhm-log-save stepper-save";
		...
		return `
            <div class="hhm-log-overlay" data-min="${stat.boundaries.min}" data-max="${stat.boundaries.max}">
                ...
                ${uiHtml}
                <div class="hhm-log-reset" data-val="${stat.boundaries.default}" title="Reset">↺</div>
	...
	private renderBadges(stat: StatConfig, habit: HabitData): string {
		const m = habit.mastery;

		if (stat.type !== "habit" || stat.multiplier === 0 || !m) return "";

		const masteryTooltip = `Level ${m.level}\nLifetime XP: ${m.totalXp}\nProgress: ${m.currentXp}/${m.requiredXp} XP`;
		const progressStyle = `width:${m.progress}%`;

		let rankHtml = "";
		if (m.rankName) {
			const rankTooltip = `${m.rankName} Rank\nNext Rank: ${m.nextRank}`;
			rankHtml = `
                <div class="hhm-rank-container" title="${rankTooltip}">
                    <div class="hhm-rank-badge ${m.rankClass}">${m.rankName}</div>
                </div>
            `;
		}
		...
		return `
            <div class="hhm-mastery-container" title="${masteryTooltip}">
                <div class="hhm-mastery-badge">Lvl ${m.level}</div>
	...
	private renderFooter(stat: StatConfig, habit: HabitData): string {
		...
		const trendIsGood =
			goal === "up" ? (habit.trend ?? 0) > 0 : (habit.trend ?? 0) < 0;
		const trendClass =
			habit.trend !== 0
				? trendIsGood
					? "hhm-trend-good"
		...
		const statsTooltip = `Current: ${currentValStr}\nPrevious: ${prevValStr}\nLifetime: ${lifetimeValStr}\nConsistency: ${habit.logs90}/90 days`;

		const streakIcon = habit.atRisk ? "⚠️" : habit.isNewPR ? "🌟" : "🔥";
		let streakHtml = "";

		// Only render the streak section if the user has it enabled for this tracker
		if (stat.streakEnabled && habit.streak > 0) {
			streakHtml = ` | <span class="${habit.atRisk ? "hhm-streak-warning" : "hhm-streak-active"}" title="All-time best: ${habit.bestStreak}">${streakIcon} ${habit.streak}</span>`;
		}

		const trendVal = habit.trend ?? 0;
		const trendText = `${trendVal > 0 ? "+" : ""}${trendVal.toFixed(0)}%`;

		return `
            <div class="hhm-footer">
                <span title="${statsTooltip}" class="hhm-stat-details" style="cursor: help;">
	...
	private renderHeatmap(stat: StatConfig, habitData: HabitData): string {
		const monthsHtml = habitData.heatmap
			.map((month: HeatmapCell[]) => {
				const cellsHtml = month
					.map((cell: HeatmapCell) => {
						...
						const color = this.renderCellColor(
							cell.value,
							habitData.maxRecorded,
							stat.color,
							stat.boundaries,
						);
	...
	/*
	 *  format raw values based on data scale and frequency
	 */
	private formatValue(stat: StatConfig, val: number): string {
		const range = stat.boundaries.max - stat.boundaries.min;
		const isLargeScale = range > 7;

		let displayNum = val;

		// Adjust for frequency
		if (stat.freq === "week" && isLargeScale) displayNum = val * 7;
		if (stat.freq === "month" && isLargeScale) displayNum = val * 30; // rough approximation
